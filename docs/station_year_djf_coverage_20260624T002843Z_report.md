# Station-Year DJF Coverage Report

Generated UTC: 2026-06-24T00:29:34+00:00

## Database

- Host: `127.0.0.1`
- Port: `5436`
- Database: `eop012`

## Run

- Calculation run ID: `station_year_djf_coverage_20260624T002843Z`
- Methodology version: `eop012-ecwt-method-v0.1.0`
- Code commit: `043fc4c3996489b673409b968dae9df329a961d9`
- Year range: `2000-2025`
- Complete threshold: `0.95`

## Summary

| Relation or Check | Rows / Value |
| --- | ---: |
| `coverage rows for this run` | 11000 |
| `complete station-years` | 1312 |
| `partial station-years` | 9395 |
| `empty station-years` | 293 |
| `valid DJF hours represented` | 7070538 |
| `rejected source rows represented` | 26430001 |

## Coverage By Year

| Year | Complete | Partial | Empty | Valid DJF Hours |
| ---: | ---: | ---: | ---: | ---: |
| 2025 | 0 | 2704 | 105 | 1261604 |
| 2024 | 426 | 2342 | 72 | 2393065 |
| 2023 | 435 | 2331 | 76 | 1896281 |
| 2022 | 325 | 1623 | 35 | 1058154 |
| 2021 | 3 | 173 | 1 | 67908 |
| 2011 | 123 | 222 | 4 | 393526 |

## Interpretation

- This is station-year coverage for the currently loaded canonical DJF table, not final plant ECWT.
- `complete` means `coverage_ratio >= complete_threshold`; this run uses the configured threshold above.
- Coverage will change as more NOAA files are downloaded and loaded.
